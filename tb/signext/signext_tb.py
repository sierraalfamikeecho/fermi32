# Decided to use code form OG tutorial.
# READER TESTBECH
#
# BRH 10/24
...
async def signext_i_type_test(dut):
    ...
    assert (
        dut.immediate.value == "00000000000000000000000001111011"
    ), f"expected 00000000000000000000000001111011, got {dut.immediate.value}"
    ...
    await Timer(1, units="ns")  # let it propagate ...
    assert (
        dut.immediate.value == "11111111111111111111111111010110"
    ), f"expected 11111111111111111111111111010110, got {dut.immediate.value}"
...
async def signext_s_type_test(dut):
    ...
    for _ in range(100):
        ...
        assert int(dut.immediate.value) - (1 << 32) == imm - (1 << 12)
